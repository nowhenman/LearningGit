...
#
# re.escape(pattern) -- если мы используем строку в качестве паттерна, то escape делает эту строку безопасной (например,
#     в адресе сайта экранирует точки, чтобы они искались как точки, а не как спец. символы.
#
...
Например, both '[()[\]{}]' and '[]()[{}]' will match a right bracket, as well as left bracket, braces, and parentheses.
...
In byte pattern(?L: ...) switches to locale depending matching, and (?a:...) switches to ASCII - only matching(default).
...
which will match with '<user@host.com>' as well as 'user@host.com', but not with '<user@host.com' nor 'user@host.com>'.
...
# \number
Matches the contents of the group of the same number. Groups are numbered starting from 1. For example, (.+) \1 matches
'the the' or '55 55', but not 'thethe'(note the space after the group). This special sequence can only be used to match
one of the first 99 groups. If the first digit of number is 0, or number is 3 octal digits long, it will not be
...
Note that formally, \b is defined as the boundary between a \w and a \W character (or vice versa), or between \w and the
beginning / end of the string.
This means that r'\bfoo\b' matches 'foo', 'foo.', '(foo)', 'bar foo baz' but not 'foobar' or 'foo3'.

By default Unicode alphanumerics are the ones used in Unicode patterns, but this can be changed by using the ASCII flag.
